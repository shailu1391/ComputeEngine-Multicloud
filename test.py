...
def RandomHandler():
		...
		if request.method == 'POST':
				...
				def append_history(file_path,s,q,d):
					fieldnames=["sno.","s","q","d"]
					next_id = get_length(file_path)
					with open(file_path,"a") as csvfile:
						writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
						writer.writerow({"sno.":next_id,"s":s,"q":q,"d":d})
				
				append_history("datahistory.csv",final_result,w,x)



		
		return render_template("tabletest.html",  tables=[final_result.to_html(classes='data')], titles=final_result.columns.values,content =elapsedtime,cost=ecost)
